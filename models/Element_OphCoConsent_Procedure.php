...
/**
 * This is the model class for table "et_ophcoconsent_procedure".
 *
 ...
 * @property Eye $eye
 * @property EtOphcoconsentProcedureProceduresProcedures $proceduress
 * @property AnaestheticType $anaesthetic_type
 * @property EtOphcoconsentProcedureAddProcsAddProcs $add_procss
 */

class Element_OphCoConsent_Procedure extends BaseEventTypeElement
{
	public $service;
	...
	{
		return 'et_ophcoconsent_procedure';
	}
	...
	public function getProcedures() {
		...
		if (Yii::app()->getController()->getAction()->id == 'create') {
			...
			foreach (EtOphcoconsentProcedureProceduresProcedures::model()->findAll('element_id=?',array($this->id)) as $proc) {
	...
	public function getAdditional_procedures() {
		...
		if (Yii::app()->getController()->getAction()->id == 'create') {
			...
			foreach (EtOphcoconsentProcedureAddProcsAddProcs::model()->findAll('element_id=?',array($this->id)) as $proc) {
				$procedures[] = $proc->proc;
			}
		}
	...
	protected function afterSave()
	{
		foreach ($_POST['Procedures_procedures'] as $procedure_id) {
			if (!EtOphcoconsentProcedureProceduresProcedures::model()->find('element_id=? and proc_id=?',array($this->id,$procedure_id))) {
				$p = new EtOphcoconsentProcedureProceduresProcedures;
		...
		foreach (EtOphcoconsentProcedureProceduresProcedures::model()->findAll('element_id=?',array($this->id)) as $p) {
		...
		if (isset($_POST['Procedures_additional'])) {
			foreach ($_POST['Procedures_additional'] as $procedure_id) {
				if (!EtOphcoconsentProcedureAddProcsAddProcs::model()->find('element_id=? and proc_id=?',array($this->id,$procedure_id))) {
					$p = new EtOphcoconsentProcedureAddProcsAddProcs;
					$p->element_id = $this->id;
					$p->proc_id = $procedure_id;
					if (!$p->save()) {
			...
			foreach (EtOphcoconsentProcedureAddProcsAddProcs::model()->findAll('element_id=?',array($this->id)) as $p) {
	...
	public function wrap($relations=array()) {
		$data = parent::wrap(array(
			'EtOphcoconsentProcedureProceduresProcedures' => 'element_id',
			'EtOphcoconsentProcedureAddProcsAddProcs' => 'element_id',
		));
