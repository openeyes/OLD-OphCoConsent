	...
	public function actionCreate() {
		...
		if (!empty($_POST)) {
			if (@$_POST['SelectBooking'] == 'unbooked') {
				return $this->redirect(array('/OphCoConsent/Default/create?patient_id='.$this->patient->id.'&unbooked=1'));
			} else if (preg_match('/^booking([0-9]+)$/',@$_POST['SelectBooking'],$m)) {
				return $this->redirect(array('/OphCoConsent/Default/create?patient_id='.$this->patient->id.'&booking_event_id='.$m[1]));
			}
		...
		if (isset($_GET['booking_event_id']) || @$_GET['unbooked']) {
			...
			if ($episode = $this->patient->getEpisodeForCurrentSubspecialty()) {
				foreach (Yii::app()->db->createCommand()
					...
					if (!Element_OphCoConsent_Procedure::model()->find('booking_event_id=?',array($row['evid']))) {
			...
			$this->event_type = EventType::model()->find('class_name=?',array('OphCoConsent'));
